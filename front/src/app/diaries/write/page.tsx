...
export default function DiaryWritePage() {
  ...
  const createFigure = (url: string, fileId: string) => {
    ...
    figure.draggable = true; // [추가] 이 설정이 있어야 handleDragStart가 호출됩니다.
    figure.className =
  ...
  const handleDragOver = (e: React.DragEvent) => {
    ...
    if (block && editorRef.current.contains(block)) {
      ...
      // [수정] 가이드라인 위치 계산
      // 1. 블록의 텍스트가 시작되는 정확한 왼쪽 위치 계산 (padding-left 고려)
      const computedStyle = window.getComputedStyle(block);
      const paddingLeft = parseFloat(computedStyle.paddingLeft);

      // 2. 가이드라인의 높이를 텍스트 높이에 맞춤 (너무 길게 나오지 않도록)
      const indicatorHeight = rect.height;

      setDropIndicator({
        top: rect.top + window.scrollY,
        left: rect.left + window.scrollX + paddingLeft - 4, // 텍스트 바로 왼쪽에 4px 여백
        height: indicatorHeight,
        visible: true,
      });

      // 드롭 위치 판정 (상단 50%면 앞, 하단 50%면 뒤)
